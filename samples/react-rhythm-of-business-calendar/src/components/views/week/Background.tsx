...
export const Background: FC<IProps> = ({ anchorDate, commands: { newEvent, setAnchorDate }, range: { start, end } }) => {
    const { palette: { themePrimary, neutralLight, neutralLighterAlt }, semanticColors: { bodyBackground } } = useTheme();
    ...
    return (
        <Stack className={styles.background} horizontal>
            {enumerateDates(start, end).map((date, idx) => {
                ...
                const includeMonthName = date.date() === 1 || idx === 0;
